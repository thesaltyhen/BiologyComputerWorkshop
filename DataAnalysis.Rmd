...
xcapture <- read_csv("Data/nifflers_capture.csv")
xrecruitment <- read_csv("Data/nifflers_recruitment.csv")
xtreasure <- read_csv("Data/nifflers_treasure.csv")


# 2. Use glimpse() to inspect each data frame
glimpse(xcapture)
glimpse(xrecruitment)
glimpse(xtreasure)
```
...
xcapture <- xcapture %>%
  mutate(site = case_when(
    ...
    TRUE ~ site))
# 3. Convert `sex` into a factor.
xcapture$sex <- as.factor(as.character(xcapture$sex))
# 4. Examine missing values in `age` and `weight_g`.  
summary(xcapture)
xcapture %>% filter(is.na(age))
xcapture %>% filter(is.na(weight_g))
# 5. Identify and handle extreme outliers (your choice how).

# Clean treasure data here
unique(xtreasure$site)
xtreasure <- xtreasure %>%
  mutate(site = case_when (
    site == "ForbiddenForest" ~ "Forbidden Forest",
    TRUE ~ site
  ))
summary(xtreasure$treasure_g)
xtreasure %>% filter(is.na(treasure_g))  
...
```{r ploting1}
source("Scripts/niffler_data_cleaning.R")
...
# We will change the names and labels with scale, and change the appearance with 
# theme
p + 
  theme_classic() +
  scale_x_continuous(name = "Body size (g)", breaks = seq(0, 2500, 250)) + 
...
# Lets make a table that we can save of our beta estimates
